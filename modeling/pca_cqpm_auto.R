...
findMinCumulative <- function(pca.result, min=0.9){
  cu <- culcalateCumulative(pca.result)
  l <- length(cu)
  position <- NULL
  for(col in 1:l){
    if(cu[col]< min) position <- col
  }
...
culcalateCumulative <- function(pca.result){
  vars <- pca.result$sdev^2
  vars <- vars/sum(vars)
  cu <- cumsum(vars)
  return(cu)
}
